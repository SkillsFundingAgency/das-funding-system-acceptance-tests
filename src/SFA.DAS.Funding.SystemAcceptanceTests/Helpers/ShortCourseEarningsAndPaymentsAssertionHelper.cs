    ...
    public class ShortCourseEarningsAndPaymentsAssertionHelper(ScenarioContext context, EarningsSqlClient earningsSqlClient, LearningSqlClient learningSqlClient)
    {
        ...
        public async Task AssertMilestoneAndCompletionEarningsStatus(string milestoneStatus, string completionStatus)
        {
            ...
            await WaitHelper.WaitForIt(() =>
            {
                ...
                {
                    var dbValidNoMilestones = earningsModel?.GetEpisode(testData.CommitmentsApprenticeshipCreatedEvent)?.EarningsProfile?.Instalments?.All(x => !x.IsPayable) ?? true;
        ...
        public async Task AssertBasicShortCourseEarningsGenerated(long ukprn = Constants.UkPrn)
        {
            ...
            var instalments = earningsModel!.GetEpisode(ukprn, expectedCourse.CourseCode).EarningsProfile.Instalments;
            ...
            var totalPrice = earningsModel.GetEpisode(ukprn, expectedCourse.CourseCode).CoursePrice;
        ...
        public async Task AssertShortCourseEarningsSetToApproved()
        {
            ...
            {
                earningsModel = earningsSqlClient.GetShortCourseEarningsEntityModel(testData.Uln);
                var dbValid = earningsModel?.GetEpisode(testData.CommitmentsApprenticeshipCreatedEvent)?.EarningsProfile != null;
            ...
            Assert.IsTrue(earningsModel!.GetEpisode(testData.CommitmentsApprenticeshipCreatedEvent).EarningsProfile.IsApproved, "Short course earnings should be approved.");
        }
        ...
        public async Task AssertShortCourseEarningsAreGeneratedWithoutDuplication(long ukprn = Constants.UkPrn)
        {
            ...
            var instalments = earningsModel!.GetEpisode(ukprn, courseCode).EarningsProfile.Instalments;
